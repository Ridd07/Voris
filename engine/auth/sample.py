...
while True:
    ...
    if not ret:
        print("Error: Could not read frame from camera. Is the camera connected?")
        break
    ...
    for (x,y,w,h) in faces:
        ...
        save_path = "engine/auth/samples/face." + str(face_id) + '.' + str(count) + ".jpg"
        cv2.imwrite(save_path, converted_image[y:y+h,x:x+w])
